...
for i in {1..30}; do
    if mc alias set myminio http://s3-artifact-storage:9000 "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD" 2>/dev/null; then
...
if mc mb myminio/data 2>/dev/null; then
    echo "Bucket 'data' created successfully"
elif mc ls myminio/data >/dev/null 2>&1; then
...
if mc anonymous set download myminio/data; then
